...
app.put('/api/staff/alerts/:id/status', async (req, res) => {
  try {
    ...
    await run('UPDATE staff_alerts SET alert_status = ? WHERE alert_id = ?', [status, req.params.id]);
...
app.get('/api/staff/referrals', async (_req, res) => {
  try {
...
app.post('/api/staff/referrals', async (req, res) => {
  try {
    const { alertId, studentId, referralType, status, notes, referredTo, studentName } = req.body;
    await run(
      'INSERT INTO referrals (alert_id, referred_to, referral_status, student_name, notes) VALUES (?, ?, ?, ?, ?)',
      [alertId ?? null, referredTo ?? referralType ?? 'sumc_counsellor', status ?? 'pending', studentName ?? (studentId ? `Student ${studentId}` : 'Anonymous'), notes ?? 'Created from staff portal']
    );
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ message: error.message });
...
app.put('/api/staff/referrals/:id', async (req, res) => {
  try {
    ...
    await run('UPDATE referrals SET referral_status = ?, notes = ? WHERE referral_id = ?', [status, notes, req.params.id]);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
app.post('/api/staff/followups', async (req, res) => {
  try {
    const { alertId, alert_id, staffId, staff_id, notes, followup_notes } = req.body;
    await run('INSERT INTO followups (alert_id, staff_id, notes) VALUES (?, ?, ?)', [alertId ?? alert_id ?? null, staffId ?? staff_id ?? null, notes ?? followup_notes ?? 'Follow-up created from staff portal']);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
app.get('/api/staff/resources', async (_req, res) => {
  try {
    const resources = await all('SELECT * FROM resources ORDER BY resource_id DESC');
    res.json(resources);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
app.post('/api/staff/messages', async (req, res) => {
  try {
    ...
    await run('INSERT INTO messages (alert_id, sender_role, recipient, content) VALUES (?, ?, ?, ?)', [alertId, senderRole, recipient, content]);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
app.get('/api/staff/messages/:alertId', async (req, res) => {
  try {
    const messages = await all('SELECT * FROM messages WHERE alert_id = ? ORDER BY message_id DESC', [req.params.alertId]);
    res.json(messages);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
